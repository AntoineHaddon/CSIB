...
# * Run time diagnostics (RTDs) for physical ocean, sea ice and ocean carbon
#   (computaion of RTDs requires nemo_rtd is true)
# * NemoVal plots
#   (production of the NemoVal plots requires nemoval_diag is true)
#
# This script is peeled off from nemo_rtd.tsk and adapted 
# to facilitate migrations (D. Yang, Sep 2020).
#===========================================================================

if [ $nemo_rtd -eq 1 ]; then
  # Get the input files for this chunk
  ln -s $OUTPUT_PATH/${cn_exp}_1m_${chunk_start_date}_${chunk_end_date}_*.nc .
  ...
  if [ $pisces_offline -eq 0 ]; then
      ...
      ln -s ${cn_exp}_1m_${chunk_start_date}_${chunk_end_date}_grid_T.nc grid_t
      ln -s ${cn_exp}_1m_${chunk_start_date}_${chunk_end_date}_grid_U.nc grid_u
      ln -s ${cn_exp}_1m_${chunk_start_date}_${chunk_end_date}_grid_V.nc grid_v
      ln -s ${cn_exp}_1m_${chunk_start_date}_${chunk_end_date}_grid_W.nc grid_w
      ...
      if [ -f "nemo_physical_rtd.nc" ]; then 
          ...
          ./$physical_rtd_exe ${chunk_start_year} ${chunk_start_month}
      ...
      if [ $nn_ice -ne 0 ]; then
        ...
        ln -s ${cn_exp}_1m_${chunk_start_date}_${chunk_end_date}_icemod.nc icemod
  ...
  if [ $nemo_carbon -eq 1 ]; then
    ...
    if [ -z "nemo_carbon_rtd.nc" ]; then 
        ./$carbon_rtd_exe ${chunk_start_year} ${chunk_start_month}
          #echo $curr_rtd_file >> $rtd_file_del_list
    else
        # Create run time diagnostics for carbon variables
        ./$carbon_rtd_exe ${chunk_start_year} ${chunk_start_month}
...
if [ $nemoval_diag -eq 1 ] ; then    
    ...
    nemoval_mod=$((chunk_end_year % nemoval_repeat_period)) 
    ...
    if [ $nemoval_mod -eq 0 ] || [ $nemoval_mod -gt $noplt_period ]; then
        ...
        if [ $nemoval_mod -eq 0 ]; then
            pltey=$chunk_end_year
        else
            nval_tmp=$((chunk_end_year + (nemoval_repeat_period - nemoval_mod)))
        ...
        nrsy=`getdef resources run_start_year`
        if [ $pltsy -ge $nrsy ]; then
        
            # get previous chunk end year
            prcey=$(printf "%04d" $((chunk_start_year - 1)))

            # set tmp merged file suffix
            new_int_mrg_file_suf="${OUTPUT_PATH}model_rtime_nval_tmp_${RUNID}_${pltsy}_${chunk_end_year}_"
            ...
            filetypes=$($seq_exp_home_dir/bin/nval_plt_types $nemoval_pltyml_pth/$nemoval_pltyml)

            if [ $chunk_end_year -eq $pltey ]; then
                ...
                for fl in $filetypes; do
                    ...
                    if [ $chunk_start_year -le $pltsy ]; then
                        ...
                        inpt_fl="$OUTPUT_PATH/${RUNID}_1m_${chunk_start_date}_${chunk_end_date}_${fl}.nc"
                        ...
                        inpt_fl2="$OUTPUT_PATH/${RUNID}_1m_${chunk_start_date}_${chunk_end_date}_${fl}.nc"
            ...
            elif [ $chunk_start_year -le $pltsy ]; then
                #----------------------
                ...
                for fl in $filetypes; do
                    ...
                    outpt_fl="${new_int_mrg_file_suf}${fl}.nc"
                    strt="$pltsy-01-01T00:00:00"
                    end="$chunk_end_year-12-31T23:59:59"
