...
if [ "0" -eq "0" ] ; then
  ...
  aprun -j 1 -n $n_rebuild -N $n_rebuild -d ${OMP_NUM_THREADS} ${task_work_dir}/rebuild_nemo_mpi.exe
  wait
fi
...
if [ $with_nemo_diag -eq 1 ]; then
   ...
   # rebuild diaptr file
   aprun -j 1 -n 1 -N 1 -d ${OMP_NUM_THREADS} ${task_work_dir}/rebuild_nemo_mpi.exe
