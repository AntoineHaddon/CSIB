...
#  include "top_substitute.h90"
   ...
   !! $Id: p4zlim.F90 3294 2012-01-28 16:44:18Z rblod $ 
...
CONTAINS
   ...
   SUBROUTINE p4z_lim( kt )
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               zlim1  = trn(ji,jj,jk,jpno3)  / ( trn(ji,jj,jk,jpno3) + 0.1E-6_wp * 6.6_wp ) ! <CMOC OR 10/29/2013> no more nutrient limitation, this needs to be reajusted later ( trn(ji,jj,jk,jpno3) * concnnh4 + trn(ji,jj,jk,jpnh4) * conc0 )    &
                  ! &   / ( conc0 * concnnh4 + concnnh4 * trn(ji,jj,jk,jpno3)  + conc0 * trn(ji,jj,jk,jpnh4) ) <CMOC OR 10/29/2013> 
               zlim2  = 1._wp ! <CMOC OR 01/07/2014> PISCES calcification ! trn(ji,jj,jk,jppo4) / ( trn(ji,jj,jk,jppo4) + concnnh4 )
               zlim3  = xlimnfecmoc(ji,jj)       ! <CMOC OR 01/22/2014> use the iron limitation mask instead of trn(ji,jj,jk,jpfer) / ( trn(ji,jj,jk,jpfer) + concfebac )
               ...
               xfracal(ji,jj,jk) = caco3r * MIN( zlim1, zlim2, zlim3 )                  & ! <CMOC OR 01/22/2014> changes xlimnfecmoc for zlim3 ! <CMOC OR 01/07/2014> PISCES calcification ! caco3r * MIN( zlim1, zlim2, zlim3 )
                  &                       * ztem1 / ( 0.1 + ztem1 )                     &
   ...
   SUBROUTINE p4z_lim_init
      ...
      !INTEGER  ::   ji, jj, jk  ! <CMOC OR 01/22/2014>
      ...
      ! <CMOC OR 01/22/2014>
      !  WRITE(numout,*) ' Initialize the Iron limitation mask to 1 on time step 1'
      !  WRITE(numout,*) ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
      !  DO jj = 1, jpj
      !     DO ji = 1, jpi
      !
      !        ! Iron limitation mask initialization to 1  ! <CMOC OR 01/21/2014>
      !        xlimnfecmoc(ji,jj) = 1._wp               ! <CMOC OR 01/21/2014> 
      !
      !    END DO
      !  END DO
