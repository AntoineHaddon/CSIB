...
# endif
      ...
      DO jk = 1, 11 ! <CMOC OR 12/11/2013> N2 fixation integration over the surface layer, between k level 1 and k level 11
                ...
                !
                &                 * qsr(:,:)*0.43_wp* exp ( - ( (0.04 + 0.03 * trn(:,:,1,jpnch) * 1e6_wp) * fsdept(:,:,jk) ) ) / inf_cmoc & ! <CMOC OR 10/15/2015> fix the attenuation bug that substituted water attenuation with a constant factor ~0.96  ! <CMOC OR 02/05/2015> experimenting with CMOC light attenuation, i.e. linear dependence with chlorophyll (using surface/1st layer chlorophyll/1st 5 m) and surface SW radiation qsr ! <CMOC OR 03/10/2014> "softwired" parameters replace hardwired ones ! <CMOC OR 12/11/2013>  Ipar(0)/INfmax * exp ( -( kw + kchl * chl ) * z) ! <CMOC OR 12/18/2013> Ipar(0)/INfmax * exp ( -( kw + kchl * chl ) * z) replaced by Ipar_Pisces(z)/INfmax
                ! <CMOC OR 02/05/2015> see line above &                 * etot(:,:,jk) / inf_cmoc & ! <CMOC OR 03/10/2014> "softwired" parameters replace hardwired ones ! <CMOC OR 12/11/2013>  Ipar(0)/INfmax * exp ( -( kw + kchl * chl ) * z) ! <CMOC OR 12/18/2013> Ipar(0)/INfmax * exp ( -( kw + kchl * chl ) * z) replaced by Ipar_Pisces(z)/INfmax
                !
