PROGRAM rebuild_nemo
   ...
   !!
   !!  Modifies the below script to use mpi. It expects multiple namelists - 
   !!  one per mpi rank - labelled with a two digit integer suffix (eg. nam_rebuild00,
   !!  nam_rebuild01...). Each namelist would describe a different type of file to
   !!  rebuild (e.g. grid_t, grid_u..._). Open-mp is still supported.
   !!
...
!$ USE omp_lib           ! Note OpenMP sentinel
   ...
   character(len=32) :: namfile
   integer  ::  num_procs, id, ierr
...
!--------------------------------------------------------------------------------
  ...
  if ( id == 0 ) then
!    call timestamp ( )
    write ( *, '(a)' ) ' Parallel Rebuild of NEMO files'
    write ( *, '(a)' ) ' '
...
!1. Read in the namelist 
   !per tile name   
   write(namfile, '("nam_rebuild", i2.2)') &
   id

   dims(:) = ""
   nchunksize = 0
   OPEN( UNIT=numnam, FILE=namfile, FORM='FORMATTED', STATUS='OLD' )
