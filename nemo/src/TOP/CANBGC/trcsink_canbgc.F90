  ...
  SUBROUTINE canoe_sink ( kt, jnt, Kbb, Kmm, Krhs )
      ...
      DO jj = 1, jpj
         DO ji = 1,jpi
            ...
            tr(ji,jj,ikt,jrcal, Kbb) = tr(ji,jj,ikt,jrcal, Kbb) - zcalbotflx / e3t(ji,jj,ikt, Kmm)
            ! add DIC/TA to bottom layer if dissolution and to surface layer if burial
            tr(ji,jj,ikt,jqdic, Kbb) = tr(ji,jj,ikt,jqdic, Kbb) + zcalbotflx * (1.-zfactcal) * 1.E-6 / e3t(ji,jj,ikt, Kmm)
            tr(ji,jj,1,jqdic, Kbb) = tr(ji,jj,1,jqdic, Kbb) + zcalbotflx * zfactcal * 1.E-6 / e3t(ji,jj,1, Kmm)
            tr(ji,jj,ikt,jqtal, Kbb) = tr(ji,jj,ikt,jqtal, Kbb) + zcalbotflx * (1.-zfactcal) * 2.E-6 / e3t(ji,jj,ikt, Kmm)
            tr(ji,jj,1,jqtal, Kbb) = tr(ji,jj,1,jqtal, Kbb) + zcalbotflx * zfactcal * 2.E-6 / e3t(ji,jj,1, Kmm)
         ENDDO
      ENDDO
