...
! https://forge.nemo-ocean.eu/nemo/nemo/-/blob/main/src/TOP/PISCES/P4Z/p4zflx.F90
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            tra(ji,jj,1,jqdic) = tra(ji,jj,1,jqdic) + zco2flx(ji,jj) / e3t_n(ji,jj,1) * qfact2
            
            ! Compute O2 flux 
            zfld16 = satmo2g(ji,jj) * K0O2(ji,jj) * tmask_bgc_closea(ji,jj,1) * zkgo2(ji,jj)      ! (mol/L) * (m/s)
            zflu16 = trn(ji,jj,1,jqoxy) * no3_sf * tmask_bgc_closea(ji,jj,1) * zkgo2(ji,jj)       ! (mol/L) * (m/s)

            zo2flx(ji,jj)  = ( zfld16 - zflu16 ) * tmask_bgc_closea(ji,jj,1)
            tra(ji,jj,1,jqoxy) = tra(ji,jj,1,jqoxy) + zo2flx(ji,jj) / e3t_n(ji,jj,1) * qfact2 / no3_sf
      ...
      CALL iom_put("DpO2" , ( satmo2g(:,:) - trn(:,:,1,jqoxy) * no3_sf / ( K0O2(:,:) + rtrn ) )  * tmask_bgc_closea(:,:,1) )
      CALL iom_put("pO2"  ,                  trn(:,:,1,jqoxy) * no3_sf / ( K0O2(:,:) + rtrn )  * tmask_bgc_closea(:,:,1) )
      ! Carbonate system
