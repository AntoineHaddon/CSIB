      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               lpc = MAX(trn(ji,jj,jk,jrdia),0.)
               lpn = MAX(trn(ji,jj,jk,jrdn),0.)
               lpf = MAX(trn(ji,jj,jk,jrdfe),0.)
               chl = MAX(trn(ji,jj,jk,jrdch),0.)
               szc = MAX(trn(ji,jj,jk,jrzoo),0.)
               itfc=1./(lpc+szc+rtrn)
...
! assume grazing hyperbola is determined by total food concentration and the two food types are consumed in proportion to their concentrations (in C units)
               grazt=gmax2*(1.-EXP(-apl*(lpc+szc)))*trn(ji,jj,jk,jrmes)*xstepb
...
! calculate zooplankton respiration (in carbon units)
               R = MAX(zsr2*Tf*trn(ji,jj,jk,jrmes)*xstepb-cxs,0.)

               !   Update the arrays TRA which contain the biological sources and sinks
               tra(ji,jj,jk,jqdic) = tra(ji,jj,jk,jqdic) + (R + cxs)*1.E-6
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) + (R*rr_n2c + nxs1 + nxs2)*1.E-6
               tra(ji,jj,jk,jqoxy) = tra(ji,jj,jk,jqoxy) - R - cxs
               tra(ji,jj,jk,jrnh4) = tra(ji,jj,jk,jrnh4) + R*rr_n2c + nxs1 + nxs2
               tra(ji,jj,jk,jrfer) = tra(ji,jj,jk,jrfer) + R*rr_fe2c + fexs1 + fexs2
               tra(ji,jj,jk,jrzoo) = tra(ji,jj,jk,jrzoo) - grazz
               tra(ji,jj,jk,jrmes) = tra(ji,jj,jk,jrmes) + lambda2*(grazz+grazp) - R
               tra(ji,jj,jk,jrdia) = tra(ji,jj,jk,jrdia) - grazp - cxs
               tra(ji,jj,jk,jrdn) = tra(ji,jj,jk,jrdn) - grazp*rr_n2c - (nxs1+nxs2)
               tra(ji,jj,jk,jrdfe) = tra(ji,jj,jk,jrdfe) - grazp*rr_fe2c - (fexs1+fexs2)
               tra(ji,jj,jk,jrdch) = tra(ji,jj,jk,jrdch) - (grazp+cxs)*chl/(lpc+rtrn)
               tra(ji,jj,jk,jrgoc) = tra(ji,jj,jk,jrgoc) + (1.-lambda2)*(grazz+grazp)
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               spc = MAX(trn(ji,jj,jk,jrphy),0.)
               spn = MAX(trn(ji,jj,jk,jrnn),0.)
               spf = MAX(trn(ji,jj,jk,jrnfe),0.)
               chl = MAX(trn(ji,jj,jk,jrnch),0.)
...
! Micrograzer functional response is determined by phytoplankton C
               grazp=gmax1*(1.-EXP(-aps*spc))*trn(ji,jj,jk,jrzoo)*xstepb
...
! calculate "excess" relative to grazer RR
               ...
               fexs2= csw1*fexs2
! calculate zooplankton respiration (in carbon units)
               R = MAX(zsr1*Tf*trn(ji,jj,jk,jrzoo)*xstepb-cxs,0.)
               ...
               !   Update the arrays TRA which contain the biological sources and sinks
               !   --------------------------------------------------------------------
               tra(ji,jj,jk,jrzoo) = tra(ji,jj,jk,jrzoo) + lambda1*grazp - R
               tra(ji,jj,jk,jrphy) = tra(ji,jj,jk,jrphy) - grazp - cxs
               tra(ji,jj,jk,jrnn) = tra(ji,jj,jk,jrnn) - grazp*rr_n2c - (nxs1+nxs2)
               tra(ji,jj,jk,jrnfe) = tra(ji,jj,jk,jrnfe) - grazp*rr_fe2c - (fexs1+fexs2)
               tra(ji,jj,jk,jrnch) = tra(ji,jj,jk,jrnch) - (grazp+cxs)*chl/(spc+rtrn)
               tra(ji,jj,jk,jrpoc) = tra(ji,jj,jk,jrpoc) + (1.-lambda1)*grazp
            END DO
         END DO
      END DO
      !
...
! calculate "excess" relative to grazer RR
               ...
               tra(ji,jj,jk,jqdic) = tra(ji,jj,jk,jqdic) + cxs*1.E-6
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) + (nxs1 + nxs2)*1.E-6
               tra(ji,jj,jk,jqoxy) = tra(ji,jj,jk,jqoxy) - cxs
               tra(ji,jj,jk,jrphy) = tra(ji,jj,jk,jrphy) - zmortp - cxs
               tra(ji,jj,jk,jrnn) = tra(ji,jj,jk,jrnn) - zmortp*rr_n2c - (nxs1+nxs2)
               tra(ji,jj,jk,jrnfe) = tra(ji,jj,jk,jrnfe) - zmortp*rr_fe2c - (fexs1+fexs2)
               tra(ji,jj,jk,jrnch) = tra(ji,jj,jk,jrnch) - (zmortp+cxs) * thetac - chl*chldegr*xstepb
               tra(ji,jj,jk,jrnh4) = tra(ji,jj,jk,jrnh4) + nxs1 + nxs2
               tra(ji,jj,jk,jrfer) = tra(ji,jj,jk,jrfer) + fexs1 + fexs2
               tra(ji,jj,jk,jrzoo) = tra(ji,jj,jk,jrzoo) - zmortz
               tra(ji,jj,jk,jrpoc) = tra(ji,jj,jk,jrpoc) + zmortp + zmortz
! Calcification
               tra(ji,jj,jk,jqdic) = tra(ji,jj,jk,jqdic) - picfrx*(zmortp + zmortz)*1.E-6
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) - 2.*picfrx*(zmortp + zmortz)*1.E-6
               tra(ji,jj,jk,jrcal) = tra(ji,jj,jk,jrcal) + picfrx*(zmortp + zmortz)
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               !     ------------------------
               spc = MAX(trn(ji,jj,jk,jrdia),0.)
               spn = MAX(trn(ji,jj,jk,jrdn),0.)
               spf = MAX(trn(ji,jj,jk,jrdfe),0.)
               szc = MAX(trn(ji,jj,jk,jrmes),0.)
               chl = MAX(trn(ji,jj,jk,jrdch),0.)
...
! calculate "excess" relative to grazer RR
               ...
               tra(ji,jj,jk,jqoxy) = tra(ji,jj,jk,jqoxy) - cxs
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) + (nxs1 + nxs2)*1.E-6
               tra(ji,jj,jk,jrdia) = tra(ji,jj,jk,jrdia) - zmortp - cxs 
               tra(ji,jj,jk,jrdn) = tra(ji,jj,jk,jrdn) - zmortp*rr_n2c - (nxs1+nxs2)
               tra(ji,jj,jk,jrdfe) = tra(ji,jj,jk,jrdfe) - zmortp*rr_fe2c - (fexs1+fexs2)
               tra(ji,jj,jk,jrdch) = tra(ji,jj,jk,jrdch) - (zmortp+cxs) * thetac - chl*chldegr*xstepb
               tra(ji,jj,jk,jrnh4) = tra(ji,jj,jk,jrnh4) + nxs1 + nxs2
               tra(ji,jj,jk,jrfer) = tra(ji,jj,jk,jrfer) + fexs1 + fexs2
               tra(ji,jj,jk,jrmes) = tra(ji,jj,jk,jrmes) - zmortz 
               tra(ji,jj,jk,jrgoc) = tra(ji,jj,jk,jrgoc) + zmortp + zmortz
            END DO
         END DO
      END DO
      !
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               znitro2dep = MAX(0., 0.4*(6.-trn(ji,jj,jk,jqoxy))/(oxymin + trn(ji,jj,jk,jqoxy)))
               znitro2dep = MIN(1., znitro2dep )
               zonitr = nitrif * zstep * trn(ji,jj,jk,jrnh4) / (1.+ par_3bands(ji,jj,jk)) * (1.- znitro2dep) 
               !denitnh4(ji,jj,jk) = nitrif * zstep * trn(ji,jj,jk,jpnh4) * nitrfac(ji,jj,jk) 
               !   Update of the tracers trends
               !   ----------------------------
               tra(ji,jj,jk,jrnh4) = tra(ji,jj,jk,jrnh4) - zonitr
               tra(ji,jj,jk,jqno3) = tra(ji,jj,jk,jqno3) + zonitr
               tra(ji,jj,jk,jqoxy) = tra(ji,jj,jk,jqoxy) - 2. * zonitr
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) - 2.e-6 * zonitr
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               zorem  = xremik * xstepb * Tf * trn(ji,jj,jk,jrpoc)
               zofer  = zorem * rr_fe2c
               zorem2 = xremik * xstepb * Tf * trn(ji,jj,jk,jrgoc)
...
! NO3 fraction is then divided between NO3 and NH4 according to the parameter nh4frx (for anammox 50% of N comes from NO3 and 50% from NH4)
               zonitr=1.-MIN(trn(ji,jj,jk,jqoxy),o2thresh)/o2thresh
               tra(ji,jj,jk,jrnh4) = tra(ji,jj,jk,jrnh4) + (zorem + zorem2)*rr_n2c - (zorem + zorem2)*nyld*zonitr*0.5*nh4frx
               tra(ji,jj,jk,jqno3) = tra(ji,jj,jk,jqno3) - (zorem + zorem2)*nyld*zonitr*(1.-0.5*nh4frx)
               tra(ji,jj,jk,jqoxy) = tra(ji,jj,jk,jqoxy) - (zorem + zorem2)*(1.-zonitr)
               tra(ji,jj,jk,jrfer) = tra(ji,jj,jk,jrfer) + zofer + zofer2
               tra(ji,jj,jk,jqdic) = tra(ji,jj,jk,jqdic) + (zorem + zorem2)*1.e-6
               tra(ji,jj,jk,jrpoc) = tra(ji,jj,jk,jrpoc) - zorem
               tra(ji,jj,jk,jrgoc) = tra(ji,jj,jk,jrgoc) - zorem2
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) + 1.e-6 * (zorem + zorem2)*rr_n2c                         ! 1 mol of alkalinity per mol of N
               tra(ji,jj,jk,jqtal) = tra(ji,jj,jk,jqtal) + 1.e-6 * (zorem + zorem2)*nyld*zonitr*(1.-nh4frx)        ! +1 mol if denitrification, 0 if anammox
...
! irreversible scavenging as in Christian et al 2002
               zcoag = MIN((trn(ji,jj,jk,jrpoc)+trn(ji,jj,jk,jrgoc))*pocfctr,1.)
               fexs = MAX(trn(ji,jj,jk,jrfer)-ligand,0.)
               zscave = xlam1 * xstepb * (trn(ji,jj,jk,jrfer)-fexs) * zcoag
               zscavex = xlam2 * xstepb * fexs
               tra(ji,jj,jk,jrfer) = tra(ji,jj,jk,jrfer) - (zscave+zscavex)
            END DO
         END DO
      END DO
      !
