...
!DIR$ NOOPTIMIZE
  ...
  use mpi
#endif
  use event_mod, only : event_t
  use canam_layer_mod, only : format_ibuf
...
#  define _IMPI_ 4
  ...
  integer(kind=impi), public, save :: intercomm_cpl_atm = -1 !< Communicator between coupler and atmosphere groups
  integer(kind=impi), public, save :: intercomm_cpl_ocn = -1 !< Communicator between coupler and ocean groups
  integer(kind=impi), public, save :: intercomm_atm_ocn = -1 !< Communicator between atmosphere and ocean groups
