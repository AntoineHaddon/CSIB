...
module com_cpl
  ...
  use mpi_types_mod, only : group_info_t, id_coupler, id_ocean, id_atmosphere
  ...
  public :: nlon_a_with_olap, nlon_o_with_olap
  public :: nlon_canom, nlat_canom, olap_canom
  public :: size_atm_grid, size_ocn_grid, size_ice_grid, size_canom_grid
  public :: a_mask, a_flnd
  ...
  public :: init_common_coupler_parameters
  public :: recv_fld, send_fld
  public :: recv_data_rec, send_data_rec
  public :: bcastGroup
  public :: bcast_inter

  !> Receive an array from a given rank by record name
  ...
  !> Send an array to a given rank by record name
  ...
  !> Receive an array from a given rank by field id
  interface recv_fld
  ...
  !> Send an array to a given rank by field id
  interface send_fld
  ...
  !> Broadcast the given field from a single task to all tasks in an MPI_COMM_WORLD
  ...
  !> Broadcast the field from single task to a named communicator
  ...
  !> Print summary statistics of the given variable to stdout
  interface print_var_stats
    module procedure print_var_stats_data_nwrds
    module procedure print_var_stats_data
  end interface

  !> Return summary statistics of the given variable as a string
  interface sprint_var_stats
    module procedure sprint_var_stats_data_nwrds
    module procedure sprint_var_stats_data
  end interface

  ! macro to string-ize variables
  !     - we should re-evaluate if this is needed
#if defined(CrayFTN) || defined(__GFORTRAN__)
#define QUOTEMAC(x) "x"
#else
#define QUOTEMAC(x) #x
#endif
...
#  ifdef coupler_specified_bc_file
  !--- If the cpp macro for "coupler_specified_bc_file" has a value then
  !--- assume it is the name of a file containing climatolgies
  !--- for GT, SIC and SICN, and 'stringize' it here.
...
#  define _IMPI_ 4
  ...
  !> Coupling frequencies (in seconds) for each component model
  ...
  integer, parameter, public :: cpl_gid = 1    !> Group id for the coupler
  integer, parameter, public :: ocn_gid = 2    !> Group id for the ocean component
  integer, parameter, public :: atm_gid = 3    !> Group id for the atmospheric component
  integer, parameter, public :: ice_gid = 4    !> Group id for the ice model
  integer, parameter, public :: msg_gid = 5    !> Group id for the msg???
  integer, parameter, public :: canom_gid = 6    !> Group id for the CanOM ocean component
  integer, parameter, public :: other_gid = 7    !> Group id for another, unknown component

  !--- CTEM dimension sizes
  integer, parameter :: ictem=9 !> number of CTEM plant functional types
  integer, parameter ::  ignd=3 !> number of SOIL layers
  integer, parameter ::  ican=4 !> number of CLASS plant functional types
  ...
  integer, save :: nevents = 0 !< The number of events that occur between components
  ...
  !> Hold information about the type of remapping
  ...
  !> Define the default options used for regridding
  ...
  type(group_info_t), dimension(3) :: group_info !< Information about each gorup
  type(group_info_t) :: my_group                 !< Holds the information for the group that this task belongs to
  !> model_group_comm is the group intra-communicator returned by mpi_comm_split
  integer(kind=impi), public, save :: model_group_comm =  MPI_COMM_NULL

  !> True if the MPI communicator has been initialized
  logical, save :: com_mpi_initialized = .false.

  !> Default value to set as a fill value
  ...
  real(kind=8), pointer, public :: nemo_glamt(:,:) !< Longitudes at the T-point of the NEMO grid
  real(kind=8), pointer, public :: nemo_glamu(:,:) !< Longitudes at the U-point of the NEMO grid
  real(kind=8), pointer, public :: nemo_glamv(:,:) !< Longitudes at the V-point of the NEMO grid
  real(kind=8), pointer, public :: nemo_glamf(:,:) !< Longitudes at the F-point of the NEMO grid

  !--- nemo_gphit, nemo_gphiu, nemo_gphiv, nemo_gphif are cell center latitudes
  !--- (in degrees) on the global NEMO T,U,V,F grids respectively
  real(kind=8), pointer, public :: nemo_gphit(:,:) !< Latitudes at the T-point of the NEMO grid
  real(kind=8), pointer, public :: nemo_gphiu(:,:) !< Latitudes at the U-point of the NEMO grid
  real(kind=8), pointer, public :: nemo_gphiv(:,:) !< Latitudes at the V-point of the NEMO grid
  real(kind=8), pointer, public :: nemo_gphif(:,:) !< Latitudes at the F-point of the NEMO grid
  ...
  real(kind=8), pointer, public :: nemo_e1t(:,:) !< Distance (in meters) along the zonal direction between T points
  real(kind=8), pointer, public :: nemo_e1u(:,:) !< Distance (in meters) along the zonal direction between U points
  real(kind=8), pointer, public :: nemo_e1v(:,:) !< Distance (in meters) along the zonal direction between V points
  real(kind=8), pointer, public :: nemo_e1f(:,:) !< Distance (in meters) along the zonal direction between F points
  real(kind=8), pointer, public :: nemo_e2t(:,:) !< Distance (in meters) along the meridional direction between T points
  real(kind=8), pointer, public :: nemo_e2u(:,:) !< Distance (in meters) along the meridional direction between U points
  real(kind=8), pointer, public :: nemo_e2v(:,:) !< Distance (in meters) along the meridional direction between V points
  real(kind=8), pointer, public :: nemo_e2f(:,:) !< Distance (in meters) along the meridional direction between F points

  !--- nemo_tmask, nemo_umask, nemo_vmask, nemo_fmask
  !--- are the global ocean/land mask on the NEMO T,U,V,F grids respectively
  real(kind=8), pointer, public :: nemo_tmask(:,:) !< Ocean/land mask on the T grid
  real(kind=8), pointer, public :: nemo_umask(:,:) !< Ocean/land mask on the U grid
  real(kind=8), pointer, public :: nemo_vmask(:,:) !< Ocean/land mask on the V grid
  real(kind=8), pointer, public :: nemo_fmask(:,:) !< Ocean/land mask on the F grid

  integer(kind=4), save, public :: nemo_n_send_var=0
  character(32),   pointer, public :: nemo_send_var(:) !< A list of variable names sent from NEMO to the coupler
                                                       !! This list is defined at run time in nemo subroutine
                                                       !! cpl_cancpl_define
  integer(kind=4), save, public :: nemo_n_recv_var=0
  character(32),   pointer, public :: nemo_recv_var(:) !< A list of variable names sent from the coupler to NEMO
                                                       !! This list is defined at run time in nemo subroutine
                                                       !! cpl_cancpl_define


  integer(kind=4), save, public :: atm_n_send_var=0
  character(32),   pointer, public :: atm_send_var(:) !< A list of variable names sent from the AGCM to the coupler
                                                      !! This list is defined at run time in the subroutine
                                                      !! define_atm_send_var
  ...
  character(32),   pointer, public :: atm_recv_var(:) !< A list of variable names sent from the coupler to the AGCM
                                                      !! This list is defined at run time in the subroutine
                                                      !! define_atm_send_var
  ...
  character(128), public :: cpl_rs_file_name_in   = "cpl_restart.nc" !< Filename of the restart file to be read in
  character(128), public :: cpl_rs_file_name_out  = "cpl_restart_out.nc" !< Filename of the restart file to be output
  ...
  character(128), public :: atm_grid_desc = " "    !< File describing the grid of the atmosphere
  character(128), public :: ocn_grid_desc = " "    !< File describing the grid of the ocean
  character(128), public :: ice_grid_desc = " "    !< File describing the grid of the ice model
  character(128), public :: canom4_grid_desc = " " !< File describing the grid of the old ocean model
  ...
  !> If true, O_Runoff be zeroed out before it is sent to NEMO
  ...
  logical, public       :: landfrac_bug = .false. !< If true, retain bug in atmospheric land mask
  character(32) , public :: windstress_remap = 'bilinear' !< Method used to remap stress from atmosphere to ocean

  !> Namelist containing parameters that must be set for the main coupler program
  !! Values should be moved to comm_coupler_par if they need to be accessed/set across
  !! multiple programs (i.e. coupler, ocean, and agcm)
  public :: main_coupler_par
  namelist /main_coupler_par/ atm_grid_desc, ocn_grid_desc, ice_grid_desc, &
        canom4_grid_desc, canesm4_cpl_restart, nemo_mesh_mask_file, agcm_grid_area_file, &
        cpl_use_initial_conditions, env_model, env_start, env_runid, &
        ...
        cpl_hist_file_name_to_read, landfrac_bug, windstress_remap

  !> Namelist containing parameters that need to set/accessed across multiple programs that use this
  !! module
  public :: comm_coupler_par
  namelist /comm_coupler_par/ specified_bc, specified_bc_file, couple_serial, coupler_parallel_type, &
                    specified_bc_year_offset, specified_bc_years_in_loop, atm_forcing_from_file

  !> size_char_msg is the size of message that will be passed in a
  !! single mpi send or receive when using the MSG array
  integer(kind=4), parameter :: size_char_msg = 8

  ! horizontal dimension sizes
  integer(kind=4), save :: olap_a           = 1     !< Number of points that overlap in the zonal direction for the atmosphere
  integer(kind=4), save :: nlon_a           = 128   !< Number of points in the zonal direction for the atmosphere
  integer(kind=4), save :: nlon_a_with_olap         !< Number of points in the zonal direction for the atm. including overlap points
  integer(kind=4), save :: nlat_a           = 64    !< Number of points in the meridional direction for the atmosphere
  integer(kind=4), save :: olap_o           = 2     !< Number of points that overlap in the zonal direction for the ocean
  integer(kind=4), save :: nlon_o           = 360   !< Number of points in the zonal direction for the ocean
  integer(kind=4), save :: nlon_o_with_olap         !< Number of points in the zonal direction for the ocean including overlap points
  integer(kind=4), save :: nlat_o           = 292   !< Number of points in the meridional direction for the ocean
  integer(kind=4), save :: olap_i           = 0     !< Number of points in the halo region for the ice
  integer(kind=4), save :: nlon_i           = 360   !< Number of points in the zonal direction for the ice
  integer(kind=4), save :: nlat_i           = 292   !< Number of points in the meridional direction for the ice
  integer(kind=4), save :: nlon_canom       = 256   !< deprecated - should be removed
  integer(kind=4), save :: nlat_canom       = 192   !< deprecated - should be removed
  integer(kind=4), save :: olap_canom       = 1     !< deprecated - should be removed

  public :: comm_coupler_horz_dims
  namelist /comm_coupler_horz_dims/ nlon_a, nlat_a, olap_a, nlon_o, nlat_o, olap_o, nlon_i, nlat_i, olap_i, &
        nlon_canom, nlat_canom, olap_canom  ! canom dims are slated for removal

  ! total grid sizes (get derived from horz. dims)
  integer(kind=4), save :: size_canom_grid  !< deprecated - should be removed
  integer(kind=4), save :: size_atm_grid    !< total number of grid points on the atm grid
  integer(kind=4), save :: size_ocn_grid    !< total number of grid points on the ocn grid
  integer(kind=4), save :: size_ice_grid    !< total number of grid points on the ice grid

  ! These arrays define integer tags associated with certain records
  ! that are passed via MPI
  ! Going forward, they have been replaced by cpl_var_list defined above.
  !--- Records on the atm grid
  integer(kind=8), dimension(3), save :: a_mask
  integer(kind=8), dimension(3), save :: a_flnd

  !--- member is defined here for compatability with the old recv_fld and send_fld
  !--- which should be avoided in preference for the new recv_data_rec and send_data_rec
  ...
  contains
    ...
    !
    !! By "common" it means variables that are used
    !! across multiple programs and thus need to be
    !! initialized in their various initialization
    !! sections
    !==================================================
    subroutine init_common_coupler_parameters()
      implicit none
      logical :: namelist_found

      print *, "Initializing common coupler parameters"
      call flush(6)

      !==================
      ! read in namelists
      !==================
      ! check if the namelist exists and bail out if not, else open it
      inquire(file='nl_coupler_par', exist=namelist_found)
      if ( .not. namelist_found ) then
        write(6,'(a)')'=== init_common_coupler_parameters: Namelist file nl_coupler_par is missing.'
        call flush(6)
        call err_exit("init_common_coupler_parameters",-1)
      endif
      open(96,file='nl_coupler_par',form='formatted')

      ! read in the horizontal dimensions
      read(96,comm_coupler_horz_dims)
      rewind(96)

      ! read in other config params
      read(96,comm_coupler_par)
      close(96)

      !=====================
      ! set inherited values
      !=====================
      ! longitude counts including desired number of overlapping points
      ! - these get used in the handling of the history files for the coupler
      nlon_a_with_olap = nlon_a + olap_a
      nlon_o_with_olap = nlon_o + olap_o

      ! number of elements on the atmosphere 2D grid
      size_atm_grid = nlon_a_with_olap * nlat_a

      ! number of elements on the ocean 2D grid
      size_ocn_grid = nlon_o_with_olap * nlat_o

      ! number of elements on the ice 2D grid
      size_ice_grid = (nlon_i+olap_i) * nlat_i

      ! OLD ocean 2D grid (why is this still needed?)
      size_canom_grid = (nlon_canom+olap_canom) * nlat_canom

      ! These arrays are tag arrays for specific quantities
      a_mask = (/ 170_4, 170_4, size_atm_grid /)   ! DOESN'T SEEM TO BE USED
      a_flnd = (/ 171_4, 171_4, size_atm_grid /)   ! Only used in ONE call to mpi_putggb2 (can probably be removed)

    end subroutine init_common_coupler_parameters
    ...
    function sprint_var_stats_data_nwrds(var_data, nwrds, name, pfx) result(str)
      real(kind=8)           :: var_data(:) !< Variable data to operate on
      integer                :: nwrds       !< Number of elements to operate over
      character(*), optional :: name        !< The name of the variable
      character(*), optional :: pfx         !< String to prefix the output
      character(512)         :: str         !< Output string with statistics
    ...
    subroutine define_cpl_var_list()
      ...
      atm_var_list = &
      (/ cpl_vinfo_t('DATA_atm',   'atm', 100, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "DATA", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('GC_atm',     'atm', 102, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "  GC", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('GT_atm',     'atm', 103, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "  GT", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SIC_atm',    'atm', 104, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, " SIC", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OUFS_atm',   'atm', 105, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "OUFS", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OVFS_atm',   'atm', 106, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "OVFS", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SNO_atm',    'atm', 108, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, " SNO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('UOC_atm',    'atm', 110, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, " UOC", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('VOC_atm',    'atm', 111, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, " VOC", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('UICE_atm',   'atm', 112, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "UICE", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('VICE_atm',   'atm', 113, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "VICE", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('GTX_atm',    'atm', 114, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, " GTX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLTX_atm',   'atm', 115, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SLTX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('HADP_atm',   'atm', 116, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "HADP", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('MADP_atm',   'atm', 117, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "MADP", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('YGWO_atm',   'atm', 118, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "YGWO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('YGWS_atm',   'atm', 119, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "YGWS", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('YSWO_atm',   'atm', 120, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "YSWO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('YSWS_atm',   'atm', 121, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "YSWS", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('YSWI_atm',   'atm', 122, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "YSWI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('YDEP_atm',   'atm', 123, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "YDEP", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('ROFO_atm',   'atm', 124, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "ROFO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SICN_atm',   'atm', 125, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SICN", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBEI_atm',   'atm', 126, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "OBEI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OFSG_atm',   'atm', 127, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "OFSG", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('PMSL_atm',   'atm', 128, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "PMSL", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SWMX_atm',   'atm', 129, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SWMX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('XSRF_atm',   'atm', 130, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "XSRF", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('XSFX_atm',   'atm', 131, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "XSFX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('NAOD_atm',   'atm', 159, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "XXXX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('BEGO_atm',   'atm', 162, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "BEGO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('RAIN_atm',   'atm', 163, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "RAIN", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SNOW_atm',   'atm', 164, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SNOW", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('HFLI_atm',   'atm', 165, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "HFLI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('BWGO_atm',   'atm', 166, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "BWGO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('HSEA_atm',   'atm', 167, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "HSEA", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('BEGI_atm',   'atm', 168, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "BEGI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLIM_atm',   'atm', 169, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SLIM", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLIMlw_atm',   'atm', 6101, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SLIM", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLIMsh_atm',   'atm', 6102, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SLIM", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLIMlh_atm',   'atm', 6103, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "SLIM", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('GTICE_atm',  'atm', 6104, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, " GTI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('MASK_atm',   'atm', 170, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "MASK", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('FLND_atm',   'atm', 171, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "FLND", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('UFSO_atm',   'atm', 172, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "UFSO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('VFSO_atm',   'atm', 173, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "VFSO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('UFSI_atm',   'atm', 174, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "UFSI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('VFSI_atm',   'atm', 175, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "VFSI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('FSGO_atm',   'atm', 176, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "FSGO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('FSGI_atm',   'atm', 177, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "FSGI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('CO2_atm',    'atm', 178, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "XSRF", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('CO2flx_atm', 'atm', 179, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "XSFX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('RIVO_atm',   'atm', 180, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "RIVO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('BWGI_atm',   'atm', 181, size_atm_grid, nlon_a, nlat_a, olap_a, 1, -1, "BWGI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) )  /)
      ...
      oocn_var_list = &
      (/ cpl_vinfo_t('DATA_ocn',   'ocn', 200, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "DATA", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('GTX_ocn',    'ocn', 201, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, " GTX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OUFS_ocn',   'ocn', 202, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "OUFS", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OVFS_ocn',   'ocn', 203, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "OVFS", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBEA_ocn',   'ocn', 204, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "OBEA", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBWA_ocn',   'ocn', 205, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "OBWA", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('GT_ocn',     'ocn', 206, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "  GT", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLT_ocn',    'ocn', 207, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, " SLT", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('UOC_ocn',    'ocn', 210, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, " UOC", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('VOC_ocn',    'ocn', 211, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, " VOC", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('CNV_ocn',    'ocn', 212, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, " CNV", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('PSIB_ocn',   'ocn', 213, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "PSIB", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('UISO_ocn',   'ocn', 214, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "UISO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('VISO_ocn',   'ocn', 215, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "VISO", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBEI_ocn',   'ocn', 216, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "OBEI", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('ODPX_ocn',   'ocn', 217, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "ODPX", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('ODPY_ocn',   'ocn', 218, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "ODPY", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('TR01_ocn',   'ocn', 219, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "TR01", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SST2_ocn',   'ocn', 220, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "SST2", &
                     regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('HBLM_ocn',   'ocn', 221, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "HBLM", &
                     regrid_info_t('bilinear', 'fracarea', .false.) )  /)
      ...
      ice_var_list = &
      (/ cpl_vinfo_t('DATA_ice',   'ice', 300, size_ice_grid, nlon_i, nlat_i, olap_i, 1, -1, "DATA", &
                     regrid_info_t('bilinear', 'fracarea', .false.) )  /)
      ...
      nemo_var_list = &
      (/ cpl_vinfo_t('DATA_nemo', 'ocn', 400, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OIceFrc',   'ocn', 401, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_SSTSST',  'ocn', 402, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_TepIce',  'ocn', 403, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_TepMix',  'ocn', 404, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_AlbIce',  'ocn', 405, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_AlbMix',  'ocn', 406, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OIceTck',   'ocn', 407, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OSnwTck',   'ocn', 408, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_IVelx1',  'ocn', 409, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_IVely1',  'ocn', 410, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_IVelz1',  'ocn', 411, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_IVelx1',  'ocn', 412, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_IVely1',  'ocn', 413, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_IVelz1',  'ocn', 414, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_CO2FLX',  'ocn', 415, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_OTaux1',  'ocn', 416, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_OTauy1',  'ocn', 417, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_OTauz1',  'ocn', 418, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_OTaux2',  'ocn', 419, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_OTauy2',  'ocn', 420, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_OTauz2',  'ocn', 421, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_ITaux1',  'ocn', 422, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_ITauy1',  'ocn', 423, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_ITauz1',  'ocn', 424, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_ITaux2',  'ocn', 425, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_ITauy2',  'ocn', 426, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_ITauz2',  'ocn', 427, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_QsrOce',  'ocn', 428, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_QsrIce',  'ocn', 429, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_QsrMix',  'ocn', 430, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_QnsOce',  'ocn', 431, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_QnsIce',  'ocn', 432, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_QnsMix',  'ocn', 433, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OTotRain',  'ocn', 434, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OTotSnow',  'ocn', 435, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OTotEvap',  'ocn', 436, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OIceEvap',  'ocn', 437, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OSubMPre',  'ocn', 438, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OISubMSn',  'ocn', 439, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OOEvaMPr',  'ocn', 440, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_Wind10',  'ocn', 441, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_dQnsdT',  'ocn', 442, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_dQlwdT',  'ocn',6104, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_dQshdT',  'ocn',6104, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_dQlhdT',  'ocn',6106, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_Runoff',  'ocn', 443, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OCalving',  'ocn', 444, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_TauMod',  'ocn', 445, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_AtmCO2',  'ocn', 446, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OTopMlt',   'ocn', 447, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBotMlt',   'ocn', 448, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) ), &
         cpl_vinfo_t('O_MSLP',   'ocn', 449, size_ocn_grid, nlon_o, nlat_o, olap_o, 1, -1, "NEMO", &
                     regrid_info_t('conservative', 'fracarea', .false.) )  /)
      ...
      msg_var_list = &
      (/ cpl_vinfo_t('MSG',      'msg', 900, size_char_msg, size_char_msg, 1, 0, 1, -1, " MSG", default_regrid ), &
         cpl_vinfo_t('MSG_ocn',  'msg', 901, size_char_msg, size_char_msg, 1, 0, 1, -1, "MSGO", default_regrid ), &
         cpl_vinfo_t('MSG_atm',  'msg', 902, size_char_msg, size_char_msg, 1, 0, 1, -1, "MSGA", default_regrid ), &
         cpl_vinfo_t('MSG_ice',  'msg', 903, size_char_msg, size_char_msg, 1, 0, 1, -1, "MSGI", default_regrid ), &
         cpl_vinfo_t('MSG_nemo', 'msg', 904, size_char_msg, size_char_msg, 1, 0, 1, -1, "MSGN", default_regrid )  /)
      ...
      (/ cpl_vinfo_t('Sp_Op1',    'msg', 801, 1, 1, 1, 0, 1, -1, "SpOp", default_regrid ), &
         ...
         cpl_vinfo_t('SST_canom',  'canom', 811, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, " SST", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SLT_canom',  'canom', 812, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, " SLT", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OUFS_canom', 'canom', 813, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "OUFS", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OVFS_canom', 'canom', 814, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "OVFS", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBEA_canom', 'canom', 815, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "OBEA", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('OBWA_canom', 'canom', 816, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "OBWA", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('QSOL_canom', 'canom', 817, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "QSOL", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('SICN_canom', 'canom', 818, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "SICN", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('FCOK_canom', 'canom', 819, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "FCOK", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('XCO2_canom', 'canom', 820, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "XCO2", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('P_canom',    'canom', 821, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
                     1, -1, "   P", regrid_info_t('bilinear', 'fracarea', .false.) ), &
         cpl_vinfo_t('XKW_canom',  'canom', 822, size_canom_grid, nlon_canom, nlat_canom, olap_canom, &
    ...
    subroutine add_cpl_var(new_var, exists)
      ...
      !> The new variable to be added; must be completely defined by the user
      type(cpl_vinfo_t), pointer, intent(in) :: new_var

      !> Provides the user control over whether the existing
    ...
    subroutine define_cpl_var_by_name(name, grid)
      ...
      character(*), intent(in) :: grid !< Name of the grid associated with the field
    ...
    subroutine add_events_nemo_to_cpl(nrec, tmp_event)
      !> The current number of variables defined in tmp_event
      integer :: nrec
    ...
    !> Initialize MPI-related variables. This subroutine must be called prior to using any variables, procedures
    !! etc from com_cpl
    ...
    !> Initialize the event array. A complete definition of each event will be done later
    ...
    !> Routine that returns the group_id given a group name
    integer function find_group_index( group_name ) result(group_index)
      character(len=*), intent(in) :: group_name

      group_index = -1
      select case ( TRIM(group_name) )
        case ('cpl')
          group_index = id_coupler
        case ('atm')
          group_index = id_atmosphere
        case ('ocn')
          group_index = id_ocean
        case default
          call err_exit('find_group_index',-1)
      end select
    ...
    subroutine define_group(group_name, group_comm, group_member)
      ...
      logical :: local_use_ocn, local_use_atm, local_use_cpl
      logical :: use_ocn, use_atm, use_cpl
      ...
      integer(kind=impi) :: rank, nproc, proc, color, key, comm_size, group_rank, group_id
      ...
      group_id = find_group_index(group_name)
      select case ( group_id )
        ...
        case default
          ...
          write(6,'(a)')  'define_group: Valid names are: atm cpl ocn'
          call flush(6)
      ...
      call mpi_comm_split(MPI_COMM_WORLD, group_id, key, group_comm, ierr)

      call mpi_allreduce( local_use_ocn, use_ocn, 1_impi, MPI_LOGICAL, MPI_LOR, MPI_COMM_WORLD, ierr )
      call mpi_allreduce( local_use_atm, use_atm, 1_impi, MPI_LOGICAL, MPI_LOR, MPI_COMM_WORLD, ierr )
      call mpi_allreduce( local_use_cpl, use_cpl, 1_impi, MPI_LOGICAL, MPI_LOR, MPI_COMM_WORLD, ierr )
      ...
      cpl_master = HUGE(cpl_master)
      atm_master = HUGE(atm_master)
      ice_master = HUGE(ice_master)
      ocn_master = HUGE(ocn_master)

      ! Fill out the group_info types based on the tasks in the group communicator
      select case (group_id )
        case (id_coupler)
          call group_info(id_coupler)%initialize( group_comm, id_coupler )
        case (id_atmosphere)
          call group_info(id_atmosphere)%initialize( group_comm, id_atmosphere )
        case (id_ocean)
          call group_info(id_ocean)%initialize( group_comm, id_ocean )
      end select

      ! Define the masters as the lowest rank in the group and sync the group info across all ranks
      if (use_cpl) then
        call mpi_allreduce( group_info(id_coupler)%leader, &
                            cpl_master, 1, MPI_INTEGER, MPI_MIN, MPI_COMM_WORLD, ierr )
        call group_info(id_coupler)%broadcast(cpl_master)
      endif
      if (use_ocn) then
        call mpi_allreduce( group_info(id_ocean)%leader, &
                            ocn_master, 1, MPI_INTEGER, MPI_MIN, MPI_COMM_WORLD, ierr )
        call group_info(id_ocean)%broadcast(ocn_master)
      endif
      if (use_atm) then
        call mpi_allreduce( group_info(id_atmosphere)%leader, &
                            atm_master, 1, MPI_INTEGER, MPI_MIN, MPI_COMM_WORLD, ierr )
        call group_info(id_atmosphere)%broadcast(atm_master)
      endif

      ! Save the current group for use by other routines
      my_group = group_info(group_id)
    ...
    subroutine cpl_define_global_parameters()
      ...
      if ( atm_forcing_from_file ) then
        ...
        if ( my_group%id == id_coupler ) then
          nemo_n_recv_var = 15
      ...
      if ( nemo_jpiglo > 0 .and. nemo_jpjglo > 0 ) then
        ...
        if ( rank == ocn_master .or. my_group%id == id_coupler ) then
      ...
      if ( rank == atm_master .or. my_group%id == id_coupler ) then
      ...
      if ( atm_n_send_var > 0 ) then
        if ( my_group%id /= id_atmosphere ) then
    ...
    subroutine recv_data_rec_i1d (i_data, ibuf, origin, rec_name, dbg)
      integer(kind=8),     intent(out) :: i_data(:) !< Data received (1D integer)
      integer(kind=8),     intent(out) :: ibuf(:)   !< An integer array with information about the data array
      integer (kind=impi), intent(in)  :: origin    !< The communicator ID of the incoming component
      character(*),        intent(in)  :: rec_name  !< Name of the field
      integer,             intent(in), optional :: dbg !< Set the debugging level
      !--- Local
      type(cpl_vinfo_t)         :: cpl_vinfo
      integer                   :: curr_size
    ...
    !> Receive 1D integer data array without an informational buffer
    ...
    !> Receive 2D real data array with an informational buffer
    subroutine recv_data_rec_r2d (r_data, ibuf, origin, rec_name, dbg)
      real(kind=8),        intent(out) :: r_data(:,:) !< Data array to be received (real)
      integer(kind=8),     intent(out) :: ibuf(:)     !< Integer array containing metadata
      integer (kind=impi), intent(in)  :: origin      !< Originating communicator id
      character(*),        intent(in)  :: rec_name    !< Name of the field to receive
    ...
    !> Receive a 3D real data array, not used currently
    ...
    subroutine send_data_rec_i1d (i_data, ibuf, dest, rec_name, dbg)
      integer(kind=8),     intent(in) :: i_data(:) !< Data array to be sent
      integer(kind=8),     intent(in) :: ibuf(:)   !< Metadata array
      integer (kind=impi), intent(in) :: dest      !< Receiving communicator ID
      character(*),        intent(in) :: rec_name  !< Name of the record
      integer,             intent(in), optional :: dbg !< Debugging level
      !--- Local
      type(cpl_vinfo_t) :: cpl_vinfo
      integer           :: curr_size
      real(kind=8), allocatable :: wrk(:)
    ...
    !> Send a 1D integer array and infer the array metadata
    ...
    !> Send 2D real data array with the informational metadata
    subroutine send_data_rec_r2d (r_data, ibuf, dest, rec_name, dbg)
      real(kind=8),        intent(in) :: r_data(:,:) !< Real data array to send
      integer(kind=8),     intent(in) :: ibuf(:)     !< Array metadata
      integer (kind=impi), intent(in) :: dest        !< Receiving communicator ID
      character(*),        intent(in) :: rec_name    !< Field name
      integer,             intent(in), optional :: dbg !< Debugging level
      !--- Local
      type(cpl_vinfo_t) :: cpl_vinfo
      integer           :: curr_size, dims(2)
...
!==============================================================================
    ...
    function group_leader(group_name) result(master)
      ...
      select case ( find_group_index(group_name) )
      case (id_coupler)
        master = cpl_master
      case (id_ocean)
        master = ocn_master
...
!--TODO-- Need a better way to assign these with a unique tag value
      ...
      in_group = my_group%id == find_group_index(remote_group)
      ...
      if ( in_group ) then
        ...
        remote_leader = my_group%leader
        remote_comm_world = my_group%communicator
