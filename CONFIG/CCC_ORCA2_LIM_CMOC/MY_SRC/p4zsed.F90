MODULE p4zsed
   ...
   INTEGER  :: ntimes_dust, ntimes_riv, ntimes_ndep ! <CMOC OR 01/22/2014>, ntimes_fmsk  ! <CMOC OR 01/22/2014> add ntimes_fmsk     ! number of time steps in a file
...
# if ! defined key_kriest
            ...
            trn(ji,jj,ikt,jpdoc) = trn(ji,jj,ikt,jpdoc) &
               ...
               &               + ( trn(ji,jj,ikt,jppoc) * zwsbio3 ) * zrivpo4 ! <CMOC OR 05/05/2014> Removal of GOC tracer ! 
...
!CDIR NOVERRCHK
   ...
   SUBROUTINE p4z_sed_init
      ...
      !
      INTEGER  :: ji, jj, jk, jm
      INTEGER  :: numdust, numriv, numiron, numdepo, numfmsk ! <CMOC OR 01/21/2014> add numfmsk (iron limitation mask)
      INTEGER  :: ierr, ierr1, ierr2, ierr3
      REAL(wp) :: zexpide, zdenitide, zmaskt
      REAL(wp), DIMENSION(nbtimes) :: zsteps                 ! times records
      REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: zdust, zndepo, zriverdic, zriverdoc, zcmask ! <CMOC OR 01/22/2014>, zfmask ! <OR CMOC 01/22/2014> add zfmask for iron limitation mask
      REAL(wp), DIMENSION(:,:)  , ALLOCATABLE :: zfmask                                      ! <OR CMOC 01/22/2014> add zfmask for iron limitation mask
      !
      CHARACTER(len=100) ::  cn_dir          ! Root directory for location of ssr files
      TYPE(FLD_N) ::   sn_dust, sn_riverdoc, sn_riverdic, sn_ndepo, sn_ironsed, sn_fmsk    ! <OR CMOC 01/21/2014> add sn_fmsk (iron limitation mask)        ! informations about the fields to be read
      NAMELIST/nampissed/cn_dir, sn_dust, sn_riverdic, sn_riverdoc, sn_ndepo, sn_ironsed, &
        &                sn_fmsk, ln_dust, ln_river, ln_ndepo, ln_ironsed,         &       ! <OR CMOC 02/05/2014> sn_fmsk was missing from the namelist block
