...
CONTAINS
   ...
   SUBROUTINE p4z_micro( kt )
      ...
      REAL(wp) :: zgrazp, zgrazpcmoc, zgrazm, zgrazsd
...
! <CMOC OR 05/26/2014> trimming PISCES code in p4zprod.F90  &        + xpref2d * zcompadi * quotad(ji,jj,jk) + xpref2c * zcompapoc ) / ( zfood + rtrn )
               zepshert  = 0. ! <CMOC OR 05/26/2014> trimming PISCES code in p4zprod.F90  epsher * MIN( 1., zncratio )
 ...
 ! <CMOC OR 12/20/2013> mzn=0.2 was used instead of mzd=0.05 for the linear coefficient (first term on the right of tra ) ! <CMOC OR 12/05/2013> fixed volume in 0.1_wp parameter (from m^-3 to L^-1)  !+ zmortz - zgrazm <CMOC OR 10/02/2013> contribution of microzooplankton mortality to detritus pool
               ...
               zprcaca = xfracal(ji,jj,jk) * zgrazpcmoc ! <CMOC OR 01/07/2014> ! PISCES calcification ! * zgrazp
               !
               ! calcite production
               prodcal(ji,jj,jk) = prodcal(ji,jj,jk) + zprcaca  ! prodcal=prodcal(nanophy)+prodcal(microzoo)+prodcal(mesozoo)
               !
               zprcaca = part * zprcaca
               tra(ji,jj,jk,jpdic) = tra(ji,jj,jk,jpdic) ! <CMOC OR 02/19/2014> - zprcaca      ! <CMOC OR 01/07/2014> PISCES calcification
               tra(ji,jj,jk,jptal) = tra(ji,jj,jk,jptal) ! <CMOC OR 02/19/2014> - 2. * zprcaca ! <CMOC OR 01/07/2014> PISCES calcification
