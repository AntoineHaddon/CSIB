...
!! LWS
   REAL(wp)                                 ::   rn_rice     ! sea-ice fraction relaxation timescale [days]
...
!CDIR COLLAPSE
         ...
         !
!! LWS
          ! Thickness is zero where there is no ice (lead fraction=1)
          WHERE (frld .EQ. 1)
             hicif(:,:) = 0.0
          ENDWHERE
 
          ! Set a minimum thickness of 10cm
          WHERE (frld .LT. 1 .AND. hicif(:,:) .LT. 0.1 )
             hicif(:,:) = 0.1
          ENDWHERE
         !
      ENDIF
...
!!    NAMELIST/namice_dmp/ cn_dir, ln_limdmp, sn_hicif, sn_frld
      NAMELIST/namice_dmp/ cn_dir, ln_limdmp, sn_hicif, sn_frld, rn_rice
      !!----------------------------------------------------------------------
      !
...
!!    ln_limdmp = .false.                 !* set file information (default values)
      ln_limdmp = .true.                  !* set file information (default values)
      rn_rice   = 3.0_wp
      WRITE (numout,*)'     lim_dmp_init : lim_dmp initialization ',rn_rice
...
!! LWS
         WRITE (numout,*)
         WRITE (numout,*)'         Ice fraction restoring timescale =', rn_rice, ' days'
...
!! LWS
         ...
         ! Restore to observations with the maximum rate everywhere.
         resto_ice(:,:,:) = 1._wp / ( rn_rice * 86400._wp )
