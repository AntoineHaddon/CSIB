...
!!$            IF( zh_s(ji,jk) == 0._wp )   ze_s(ji,jk) = 0._wp
      ...
      zdeltah(1:npti) = h_s_1d(1:npti) + h_i_1d(1:npti) * (rhoi-rau0) * r1_rhos
      DO jk = 0, nlay_s
         DO ji = 1, npti
            IF( zdeltah(ji) > 0._wp .AND. sst_1d(ji) > 0._wp ) THEN
               ...
               ! mass & energy loss to the ocean
               hfx_res_1d(ji) = hfx_res_1d(ji) - ze_s(ji,jk) * zdum * a_i_1d(ji) * r1_rdtice  ! heat flux to the ocean [W.m-2], < 0
               wfx_res_1d(ji) = wfx_res_1d(ji) + rhos        * zdum * a_i_1d(ji) * r1_rdtice  ! mass flux
               ! update thickness and energy
               h_s_1d(ji)    = MAX( 0._wp, h_s_1d(ji)  - zdum )
               zh_s  (ji,jk) = MAX( 0._wp, zh_s(ji,jk) - zdum )
