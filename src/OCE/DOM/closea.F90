   ...
   SUBROUTINE dom_clo()
      ...
      IF( ln_read_cfg) THEN
         ...
         IF( id > 0 ) THEN
            ...
            closea_mask(:,:) = zdata_in(:,:)
            ! The following line seems to have a problem, possibly because tmask doesn't mask the halo points and
            ! the IOM routine does return the fill values for the land-masked regions.
            ! In any case, with the default fill value of 99999, this ends up meaning that jncs ends up being 99999
            ! which is very clearly incorrect. The NINT also seems unnecessary since zdata_in is already an integer
